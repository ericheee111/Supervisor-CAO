"""Unit tests for project config loader (spec §13, §20.1)."""
...
def test_load_pandas_example():
    cfg = load_project("pandas")
    assert cfg.name == "pandas"
    assert cfg.base_branch == "dev"
    assert cfg.task_branch_prefix == "agent/"
    assert "wsl_quick" in cfg.default_verification
...
def test_list_known_projects_includes_pandas():
    projects = list_known_projects()
    assert "pandas" in projects
