...
A duration string is a sequence of one or more ``<number><unit>`` segments
with no separators, e.g. ``'1d2h30m45s'``.
"""
...
# A token is one or more digits followed by exactly one known unit letter.
_TOKEN_RE = re.compile(r"(\d+)([smhd])")
...
def parse_duration(s: str) -> int:
    ...
    Raises:
        ValueError: If *s* is empty, malformed, contains negative values,
            uses unknown units, or has a trailing number without a unit.
    """
