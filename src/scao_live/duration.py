...
suffix (``ms``, ``s``, ``m``, ``h``) into integer milliseconds. The parser is
intentionally strict: combined-unit expressions, negative values, fractional
values, unsupported units, bare numbers, and non-numeric inputs are all
...
# One non-negative integer immediately followed by exactly one supported unit.
# ``ms`` is listed before ``m``/``s`` so the two-character unit is tried first,
# avoiding a greedy single-character match that would leave a dangling trailing
# character. The ``$`` anchor rejects combined-unit strings such as ``"1m30s"``
...
def parse_duration(s: str) -> int:
    ...
    The input must be a single non-negative integer immediately followed by a
    case-insensitive unit suffix (``ms``, ``s``, ``m``, ``h``). Surrounding
