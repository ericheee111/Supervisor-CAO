...
class WorkerRunner:
    ...
    def _reviewer_prompt(task_id, candidate_sha, tested_sha, plan):
        return (
            ...
            "1. If the implementation correctly fulfills the plan, tests pass, "
            "and there are no correctness or safety issues, you MUST output APPROVED.\n"
            "2. Do NOT request performance verification, benchmarking, ARM/x86 testing, "
            "or architecture-specific results. This is STRICTLY PROHIBITED for "
            "non-performance tasks (simple utilities, string parsing, bug fixes).\n"
            "3. Do NOT request CHANGES for style preferences, minor naming, documentation, "
            "or test coverage nitpicks.\n"
            "4. Only request CHANGES for: actual correctness bugs, safety issues "
            "(e.g. path traversal), missing tests for core functionality, or "
            "incorrect behavior that causes test failures.\n"
            "5. Simple utility functions with passing tests MUST be APPROVED.\n\n"
            + _schema_hint("review")
        )
